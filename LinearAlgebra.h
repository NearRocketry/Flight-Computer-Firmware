...
vec3 vSub(vec3 const a, vec3 const b) {
    vec3 result = {a.x - b.x, a.y - b.y, a.z - b.z};
    return result;
}
...
vec3 vDiv(float const a, vec3 const b) {
...
vec3 vNorm(vec3 const a) {
    if (vLen(a) == 0) return a;
...
quat qSub(quat const a, quat const b) {
    quat result = {a.x - b.x, a.y - b.y, a.z - b.z, a.w - b.w};
    return result;
}
...
quat qDiv(float const a, quat const b) {
...
float qLen(quat const a) {
    return sqrtf(qDot(a, a));
}

quat qNorm(quat const a) {
    if (qLen(a) == 0) return a;
    return qDiv(qLen(a), a);
}
...
quat qMul(quat const a, quat const b) {
    ...
    result = qSub(result, qReal(qDot(a, b)));
    return result;
}

quat qConj(quat const a) {
    quat result = {-a.x, -a.y, -a.z, a.w};
    return result;
}

quat qPolar(vec3 axis) {
    vec3 normal = vNorm(axis);
    float angle = vLen(axis);
    return qAdd(qScale(sin(angle), qVector(normal)), qReal(cos(angle)));
}

vec3 qRot(vec3 const a, quat const b) {
    quat c = {a.x, a.y, a.z, 0};
    quat result = qMul(qMul(b, c), qConj(b));
